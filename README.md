👋 Hi, I’m Akshay (@akshaykumar-og)

🎓 MSc Management with Data Analytics graduate | Data Scientist @ Barclays

👀 Passionate about transforming data into actionable insights and building intelligent solutions

💼 Experienced in machine learning, data analysis, and end-to-end data workflows
📊 Hands-on experience improving model accuracy, automating data pipelines, and delivering business-driven insights

🛠️ Tech Stack: Python | SQL | Machine Learning | Data Visualization | Power BI | Terraform Certified associate (004)

💡 Strong interest in solving real-world problems using data and contributing to impactful, scalable solutions

🚀 Open to collaborating on Python, data science, and machine learning projects

📫 Let’s connect: [akshaykumarshekar@gmail.com](mailto:akshaykumarshekar@gmail.com) | LinkedIn


<!---
akshaykumar-og/akshaykumar-og is a ✨ special ✨ repository because its `README.md` (this file) appears on your GitHub profile.
You can click the Preview link to take a look at your changes.
--->
